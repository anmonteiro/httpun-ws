...
type frame_handler =
  opcode:Websocket.Opcode.t -> is_fin:bool -> len:int -> Payload.t -> unit
...
let advance_frame_queue t =
  ...
  then
    let { Parse.opcode; is_fin; payload_length; _ }, payload =
      Queue.peek t.frame_queue
    in
    t.frame_handler ~opcode ~is_fin ~len:payload_length payload

let rec _next_read_operation t =
  match Queue.peek t.frame_queue with
  | _, payload ->
    (match Payload.input_state payload with
    | Wait ->
    ...
    | Complete ->
      ...
      | `Error _ as op ->
        ...
        op
      | `Read ->
        (* Drain already-parsed queued frames before polling the socket again. *)
...
let read_with_more t bs ~off ~len more =
  ...
  (if not (Queue.is_empty t.frame_queue)
   then
     ...
     then try Payload.execute_read payload with exn -> report_exn t exn);
  consumed
...
let yield_writer t k =
  ...
  then (
    Wsd.close t.wsd;
    k ())
  else Wsd.on_wakeup t.wsd k

let is_closed { wsd; _ } = Wsd.is_closed wsd

let yield_reader t k =
  if Reader.is_closed t.reader then k () else Reader.on_wakeup t.reader k
