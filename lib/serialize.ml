...
let mask mode = match mode with `Client m -> Some (m ()) | `Server -> None
...
let serialize_headers ~mode faraday ~is_fin ~opcode ~payload_length =
  ...
  then Faraday.write_uint8 faraday (is_mask lor payload_length)
...
let schedule_serialize ~mode faraday ~is_fin ~opcode ~payload ~src_off ~off ~len
  =
  (match
     serialize_headers faraday ~mode ~is_fin ~opcode ~payload_length:len
   with
  | None -> ()
...
let serialize_bytes ~mode faraday ~is_fin ~opcode ~payload ~src_off ~off ~len =
  ...
  | None -> ()
  | Some mask ->
    Websocket.Frame.apply_mask_bytes mask payload ~src_off ~off ~len);
